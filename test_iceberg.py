...
async def test_paper_iceberg():
    ...
    stats = await engine.submit_twap_iceberg_order(
        symbol="BTCUSDT", side="BUY", total_quantity=5000, min_slice=500, max_slices=5
    )
...
async def test_idempotency():
    ...
    # 检查幂等性
    assert engine._check_idempotency(key1), "应检测到已执行"
    assert not engine._check_idempotency(key3), "未执行订单应返回 False"
...
async def main():
    ...
    tests = [test_ticker_fetch, test_idempotency, test_paper_iceberg]
