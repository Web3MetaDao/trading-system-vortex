...
class MultiModalOracle:
    ...
    async def _fetch_crypto_sentiment(self) -> float:
        ...
        # 检查缓存
        if self._sentiment_cache is not None and (current_time - self._cache_timestamp) < self._cache_ttl:
    ...
    def _analyze_orderbook_imbalance(self, orderbook: Dict[str, Any]) -> float:
        ...
        try:
            ...
            logger.info(
                "盘口失衡分析：bid_vol=%.2f, ask_vol=%.2f, obi=%.2f",
                bid_volume,
                ask_volume,
                obi
            )
    ...
    def evaluate_macro_environment(self, orderbook: Dict[str, Any]) -> OracleSnapshot:
        ...
        try:
            ...
            if loop.is_running():
                ...
                # 这里使用缓存值或默认值
                sentiment_score = self._sentiment_cache if self._sentiment_cache is not None else 0.0
            else:
        ...
        if is_extreme_fear and is_strong_pressure:
            ...
            logger.debug(
                "宏观环境许可：sentiment=%.2f, obi=%.2f -> 允许交易",
                sentiment_score,
                obi
            )
    ...
    async def get_oracle_snapshot_async(self, orderbook: Dict[str, Any]) -> OracleSnapshot:
        ...
        if is_extreme_fear and is_strong_pressure:
            is_trade_permitted = False
            logger.warning(
                "宏观风控触发：sentiment=%.2f, obi=%.2f -> 禁止交易",
                sentiment_score,
                obi
            )
        else:
...
if __name__ == '__main__':
    ...
    print(f"情绪得分：{snapshot.sentiment_score:.2f} ({oracle.get_sentiment_description(snapshot.sentiment_score)})")
    print(f"盘口失衡：{snapshot.orderbook_imbalance:.2f} ({oracle.get_obi_description(snapshot.orderbook_imbalance)})")
