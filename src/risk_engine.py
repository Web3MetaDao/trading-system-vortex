...
class RiskEngine:
    ...
    def can_open_position(
        ...
        trade_gate = self.trade_gate(portfolio)
    ...
    def trade_gate(self, portfolio, now_utc: datetime | None = None) -> RiskDecision:
        ...
        if consecutive_loss_pause > 0:
            ...
            if loss_streak >= consecutive_loss_pause:
                return RiskDecision(
                    False,
                    0.0,
                    f"Consecutive loss pause active: {loss_streak} losses >= limit {consecutive_loss_pause}",
                )
    ...
    def exit_reason(self, position: dict, snapshot, market_state: str) -> str | None:
        ...
        if len(klines) >= ema_exit_period:
            ...
            if ema_value and snapshot.price < ema_value and pnl_pct > 0:
                return f"ema_exit ({pnl_pct:.2f}%)"
