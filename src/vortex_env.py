...
class VortexTradingEnv(gym.Env):
    ...
    def __init__(self, df: pd.DataFrame, initial_balance: float = 1000.0):
        super(VortexTradingEnv, self).__init__()
    ...
    def _get_obs(self):
        ...
        obs = np.array([
            ...
            row['atr'], row['ema_fast'], row['ema_slow'], self.position
        ], dtype=np.float32)
        return obs
...
if __name__ == "__main__":
    # Test Environment
    df = pd.DataFrame(np.random.randn(100, 7), columns=['close', 'volume', 'rsi', 'macd', 'atr', 'ema_fast', 'ema_slow'])
