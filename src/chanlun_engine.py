...
class ChanlunEngine:
    ...
    def _find_zhongshu(self, bis: list[Bi]) -> list[ZhongShu]:
        ...
        for i in range(len(bis) - 2):
            ...
            # 检查是否形成中枢（方向交替）
            is_alternating = bi1.direction != bi2.direction and bi2.direction != bi3.direction
            ...
            if zg > zd:
                ...
                zhongshu = ZhongShu(
                    start_bi_index=i, end_bi_index=i + 2, zg=zg, zd=zd, direction=direction
                )
    ...
    def _calc_momentum(self, df: pd.DataFrame, window: int | None = None) -> pd.Series:
        """
        计算价格动量（简化版 MACD/RSI）
    ...
    def evaluate_divergence(self, df: pd.DataFrame) -> dict[str, Any] | None:
        """
        评估背驰信号（核心公共方法）
        ...
        try:
            ...
            momentum_improvement = (
                (current_momentum - prev_momentum) / abs(prev_momentum) if prev_momentum != 0 else 0
            )

            is_divergence = momentum_improvement > 0.2

            if not is_divergence:
                logger.debug("动量未出现背离：improvement=%.2f%%", momentum_improvement * 100)
                return None
    ...
    def evaluate_third_buy(self, df: pd.DataFrame) -> dict[str, Any] | None:
        """
        评估第三买点信号
...
def integrate_with_signal_engine(
    chanlun_signal: dict[str, Any] | None, current_score: int, strategy_config: dict[str, Any]
    ...
    if signal_type == "BUY":
        if pattern == "bottom_divergence":
            ...
            return (current_score + bonus, "chanlun_divergence", chanlun_signal.get("details", {}))

        elif pattern == "third_buy":
            bonus = int(strength * weight)
            return (current_score + bonus, "chanlun_3rd_buy", chanlun_signal.get("details", {}))
...
if __name__ == "__main__":
    ...
    # 运行引擎
    engine = ChanlunEngine(config={"min_bi_length": 4, "divergence_lookback": 3})
