        ...
        try:
            for symbol in symbols:
                ...
                if decision.grade in ["A", "B"]:
                    ...
                    if ml_prob > 0.6:
                        logger.info(
                            f"ML CONFIRMED [{symbol}] {decision.side} - Confidence: {ml_prob:.2f}"
                        )
                        # Dynamic Risk Sizing
                        risk_decision = risk_manager.calculate_dynamic_levels(
                            data["price"], 500.0, decision.side
                        )
                        if risk_decision.approved:
                            logger.info(
                                f"EXECUTE [{symbol}] {decision.side} Size: {risk_decision.position_size_usdt} USDT"
                            )
            time.sleep(2)
        except Exception as e:
