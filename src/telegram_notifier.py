...
class TelegramNotifier:
    ...
    async def _send_via_aiohttp(self, text: str, parse_mode: str, disable_notification: bool) -> bool:
        """通过 aiohttp 发送消息"""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=data, timeout=10) as response:
                    if response.status == 200:
                        ...
                        self._logger.error(f"Telegram API 返回错误：{response.status} - {error_text}")
                        return False
    ...
    def _send_via_requests(self, text: str, parse_mode: str, disable_notification: bool) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self._logger.error(f"Telegram API 返回错误：{response.status_code} - {response.text}")
                return False
    ...
    async def send_trade_alert(
        ...
        # 宏观罗盘
        state_emoji = {"S1": "🚀", "S2": "📈", "S3": "⚖️", "S4": "📉", "S5": "💥"}.get(market_state, "⚪")
        state_desc = {
            "S1": "强势上涨", "S2": "震荡上行", "S3": "中性震荡",
            "S4": "震荡下行", "S5": "强势下跌"
        }.get(market_state, "未知")
    ...
    def send_risk_alert(
        ...
        return self.send(text, level=TelegramAlertLevel.INFO if approved else TelegramAlertLevel.WARNING, parse_mode="HTML")
...
if __name__ == "__main__":
    ...
    lines.append(f"{emoji} <b>Trade Alert</b>")
    lines.append("━━━━━━━━━━━━━━━━━━━━")
    lines.append(f"{emoji} <b>交易对与方向:</b> <code>{test_data['symbol']}</code> | <b>{test_data['side']}</b>")
