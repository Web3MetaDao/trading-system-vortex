...
from backtest import build_snapshot, performance_from_trades  # noqa: E402
from market_data import MarketDataClient  # noqa: E402
from risk_engine_optimized import RiskEngine  # noqa: E402
from signal_engine_v2 import SignalEngineV2  # noqa: E402
...
def run_v2_backtest(symbol="BTCUSDT", limit=200):
    ...
    for i in range(30, len(klines)):
        ...
        context = {
            "snapshot": snapshot,
            "strategy": {"signal_params": {"ema_fast_period": 20, "ema_slow_period": 50}},
        }
        decision = signal_engine.evaluate(symbol, "S3", context)

        if decision.grade in ["A", "B"] and decision.side == "LONG":
            # [FIX] 通过 risk_engine 计算仓位大小（原代码实例化后未使用）
            size_info = risk_engine.size_position(
                signal_grade=decision.grade,
                stop_loss_pct=2.0,
            )
            position_usdt = size_info.get("position_size_usdt", equity * 0.1)
